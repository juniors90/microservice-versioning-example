...
class PostModelTestCase(BaseTestClass):
    ...
    def test_person_bad_request(self):
        ...
        response = self.client.post(
            "/api/v2/people", headers=self.get_api_headers(), json=data
        )
    ...
    def test_read_all(self):
        response = self.client.get("/api/v2/people")
        self.assertEqual(response.status_code, 200)

    def test_read_one(self):
        data = {"fname": "Tooth", "lname": "Fairy"}
        response = self.client.get(f'/api/v2/people/{data["lname"]}')
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_person(self):
        data = {"fname": "Tooth", "lname": "Fairy"}
        response = self.client.delete(f'/api/v2/people/{data["lname"]}')
        self.assertEqual(response.status_code, 200)
